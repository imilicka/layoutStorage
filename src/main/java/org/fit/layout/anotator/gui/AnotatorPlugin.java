 ...
 *
 */
public class AnotatorPlugin implements BrowserPlugin {
	...
	public boolean init(Browser browser) {
		...
		this.browser.addStructurePanel("Anotator", getPnl_mainPanel());
	...
	private AreaSelectionListener getListener_areaSelection () {
		...
		if(listener == null) {
			listener = new AnotatorAreaListener() { 
				@Override
	...
	private void removeTag(DefaultArea area, AnotatorTag tag) 
	{
		...
		for( int i=0; i<tagModelTable.getRowCount(); i++ ) 
		{
			...
			if(area.equals(aa) && tag.equals( new AnotatorTag(tagString) )) {
